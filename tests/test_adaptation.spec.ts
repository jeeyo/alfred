...
class SimpleQAEnvironment {
  evaluate(sample: Sample, generator_output: { final_answer: string }): EnvironmentResult {
...
describe('OfflineAdapter', () => {
  it('single step updates playbook', async () => {
    ...
    const sample: Sample = { question: 'What is the answer to life, the universe, and everything?', ground_truth: '42' };
    const environment = new SimpleQAEnvironment();
    const results = await adapter.run([sample], environment as any, 1);

    expect(results.length).toBe(1);
    expect(results[0].generator_output.final_answer).toBe('42');
    const stats = playbook.stats() as any;
